...
void display_record_to_edit(int textpos, int subjpos, int bodypos){
    ...
    while (textpos < 47 && textpos <= bodypos) {// i'm  printing out the data
        putchar(body[textpos]);
        textpos++;
    }
    xt_par2(XT_SET_ROW_COL_POS, 10, 23);
    while (textpos < 94 && textpos <= bodypos) {// i'm  printing out the data
        putchar(body[textpos]);
        textpos++;
    }
    xt_par2(XT_SET_ROW_COL_POS, 11, 23);
    while (textpos < 140 && textpos <= bodypos) {// i'm  printing out the data
        putchar(body[textpos]);
        textpos++;
    }
    xt_par2(XT_SET_ROW_COL_POS, 8, 23);
...
s
    ...
    display_record_to_edit(textpos, subjpos, bodypos);
        ...
        switch (key) {
            ...
            case KEY_BACKSPACE:
                ...
                if (cursorc > 23) {
                    ...
                    display_record_to_edit(0, subjpos, bodypos);
...
void new_entry(void) {
    ...
    while (1) {  // all the switching logic
        ...
        switch (key) {
            ...
            case KEY_F5:
                clean_up_editbox(1); //indicates is calling from new_entry()
                return;
            case KEY_RIGHT:
                if (cursorpos) {
                    if (textpos >= bodypos)
